...
from discordClient.views import Reaction, PageView123, NumbersListEmbedRender, AllAndNumbersListEmbedRender, \
    ...
    MuseumAffiliationsNumbersListEmbedRender, ViewReactionsLine, ViewWithReactions
...
class MuseumDataFilter:
    ...
    def __init__(self, owner: User, category: Any = None, rarity: List = [], affiliation: Any = None):
        self.owner = owner
        self.rarity = rarity
...
class MuseumCogs(AssignableCogs):
    ...
    async def museum(self, ctx: SlashContext, user: User = None):
        ...
        rarity_select = create_select(options=rarities_options,
                                      ...
                                      max_values=6)
        rartiy_select_line = ViewReactionsLine()
        rartiy_select_line.add_reaction(Reaction(button=rarity_select, callback=self.rarity_selected))
        ...
        for i in range(0, 23):
            affiliations_options.append(create_select_option(f"{query[i].name}",
                                                             value=f"{query[i].name}"))
        ...
        category_menu = PageView(puppet_bot=self.bot,
                                 ...
                                 lines=[rartiy_select_line, affiliation_select_line],
                                 delete_after=600,
    ...
    async def refresh_museum(self, context, menu: ViewWithReactions, museum_filter: MuseumDataFilter):
        query = Character.select(Character, fn.Count(Character.id).alias('count'), CharactersOwnership.discord_user_id)
    ...
    async def menu_categories_to_choice(self, menu_update: PageView123, user_that_reacted: User,
        ...
        if emoji is None:  # all
            ...
            hidden_data = menu_update.get_hidden_data()
    ...
    async def display_characters(self, menu_update: PageView123, user_that_reacted: User,
        ...
        query = Character.select(Character, fn.Count(Character.id).alias('count'), CharactersOwnership.discord_user_id)
        museum_filter = menu_update.get_hidden_data()
        if museum_filter.category is not None:
            query = query.where(Character.category == museum_filter.category)
        if museum_filter.rarity is not None:
            query = query.where(Character.rarity == museum_filter.rarity)
